...
function call4alntools()
{
	...
	muscle -in $infile -out $outfile'_muscle'
	mv ./$outfile'_muscle' $path
	clustalw -infile=$infile -outfile=$outfile'_clustalw' -output=phylip
	mv ./$outfile'_clustalw' $path
	t_coffee -infile=$infile -outfile=$outfile'_tcoffee' -output=phylip
